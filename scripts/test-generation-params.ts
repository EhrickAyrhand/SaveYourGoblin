...
// --- Temperature ---
async function testTemperature(generateRPGContent: (scenario: string, contentType: 'character' | 'environment' | 'mission', advancedInput?: unknown, generationParams?: unknown) => Promise<unknown>) {
  console.log('\n--- Temperature ---')
  const low: string[] = []
  const high: string[] = []
  for (let i = 0; i < 2; i++) {
...
// --- Tone ---
async function testTone(generateRPGContent: (scenario: string, contentType: 'character' | 'environment' | 'mission', advancedInput?: unknown, generationParams?: unknown) => Promise<unknown>) {
  console.log('\n--- Tone ---')
...
// --- Complexity ---
async function testComplexity(generateRPGContent: (scenario: string, contentType: 'character' | 'environment' | 'mission', advancedInput?: unknown, generationParams?: unknown) => Promise<unknown>) {
  console.log('\n--- Complexity ---')
